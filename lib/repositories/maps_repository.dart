...
class MapsRepository {
  ...
  Map<String, List<MapImage>> _parseHtml(String html) {
    ...
    for (int i = 0; i < h2Matches.length; i++) {
      ...
      final blockEnd =
          i + 1 < h2Matches.length ? h2Matches[i + 1].start : html.length;
      final block = html.substring(blockStart, blockEnd);

      final hrefs =
          hrefRegex.allMatches(block).map((m) => m.group(1)!).toList();
      final thumbs =
          thumbRegex.allMatches(block).map((m) => m.group(1)!).toList();
      ...
      for (int j = 0; j < hrefs.length; j++) {
        ...
        final thumb = thumbs
                .where((t) => t.contains('-300x') || t.contains('-268x'))
                .elementAtOrNull(j) ??
            (j < thumbs.length ? thumbs[j] : url);
